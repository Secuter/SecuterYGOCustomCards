...
function s.spfilter2(c,e,tp)
	return c:IsSetCard(SET_WYRMWIND) and c:GetLevel()>0 and c:IsCanBeEffectTarget(e) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_DEFENSE)
end
function s.sptg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return false end
	local lv=e:GetLabel()
	if chk==0 then lv=e:GetHandler():GetLevel() end
	...
	if chk==0 then return ft>1 and not Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT)
		and g:CheckWithSumEqual(Card.GetLevel,lv,2,2) end
	local tg=g:SelectWithSumEqual(tp,Card.GetLevel,lv,2,2)
