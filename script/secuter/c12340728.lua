...
function s.filter(c)
	return c:GetOriginalCode()==12340719 and c:IsAbleToHand()
end
...
function s.spfilter(c,e,tp)
	return c:IsSetCard(SET_MORHAI) and c:IsMonster() and c:IsLevelBelow(4) and not c:IsCode(id) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_DEFENSE)
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,s.spfilter,tp,LOCATION_HAND+LOCATION_GRAVE,0,1,1,e:GetHandler(),e,tp)
	if tc then
