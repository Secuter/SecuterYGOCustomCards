...
function s.negcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return rp~=tp and not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED)
		and re and re:IsActiveType(TYPE_SPELL) and Duel.IsChainNegatable(ev)
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 then
		...
		if g:IsExists(s.thfilter,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(id,3)) then
			...
			else Duel.Remove(g,POS_FACEUP,REASON_EFFECT+REASON_REVEAL)
			end
		else
