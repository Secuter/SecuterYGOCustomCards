...
function s.initial_effect(c)
	--xyz summon
	Xyz.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsSetCard,SET_ANCIENT_ORACLE),4,2)
	c:EnableReviveLimit()
	--spsummon
...
function s.filter(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(SET_ANCIENT_ORACLE) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.acfilter(c,tp)
	return c:IsCode(12341414) and c:GetActivateEffect() and c:GetActivateEffect():IsActivatable(tp,true,true)
end
