...
function s.initial_effect(c)
	--link summon
	c:EnableReviveLimit()
	Link.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsRace,RACE_MACHINE+RACE_PSYCHIC),2,2)
	--search
...
function s.cfilter(c,tp)
	return c:IsSetCard(SET_RASCAL_ACE) and c:IsMonster() and c:IsAbleToRemoveAsCost() and aux.SpElimFilter(c,true)
...
function s.thfilter(c,code)
	return c:IsSetCard(SET_RASCAL_ACE) and c:IsMonster() and not c:IsCode(code) and c:IsAbleToHand()
end
...
function s.thcost2(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,s.cfilter2,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,1,e:GetHandler())
	Duel.Remove(g,POS_FACEUP,REASON_COST)
end
