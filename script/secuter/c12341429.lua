...
function s.initial_effect(c)
	--xyz summon
	Xyz.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsSetCard,SET_ANCIENT_ORACLE),6,2,nil,nil,Xyz.InfiniteMats)
	c:EnableReviveLimit()
	--remove + disable
...
function s.filter(c)
	return c:IsAbleToRemove() and (not c:IsLocation(LOCATION_GRAVE) or aux.SpElimFilter(c,true))
end
