...
function s.initial_effect(c)
	--link summon
	Fusion.AddProcMix(c,true,true,aux.FilterBoolFunctionEx(Card.IsAttribute,ATTRIBUTE_FIRE),aux.FilterBoolFunctionEx(Card.IsType,TYPE_FUSION))
	c:EnableReviveLimit()
	--indes battle
...
function s.thfilter2(c)
	return c:IsSetCard(SET_PYROCLAST) and c:IsMonster() and not c:IsType(TYPE_FUSION) and c:IsAbleToHand()
end
