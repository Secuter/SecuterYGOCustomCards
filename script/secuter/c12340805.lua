...
function s.thfilter(c)
	return not c:IsCode(id) and c:IsAbleToHand() and ((c:IsAttribute(ATTRIBUTE_WIND) and c:IsRace(RACE_WINGEDBEAST)) or c:IsSetCard(SET_EAGLE_OVERSEER))
end
...
function s.thfilter2(c)
	return c:IsSetCard(SET_EAGLE_OVERSEER) and c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsAbleToHand()
end
